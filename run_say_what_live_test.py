...
def inject_renforge_editor(project_root: Path) -> None:
    ...
    bridge_src = REPO_ROOT / "src" / "renforge" / "bridge"
...
def prepare_fixture() -> Path:
    ...
    source = REPO_ROOT / "tests" / "fixtures" / "say_what_clean"
    ...
    try:
        ...
        with launch_with_bridge(
            ...
            scenario_report = run_editor_say_what_style_position_live_scenario(
                ...
                gui_path=gui_path,
                screenshot_path=(
                    Path(os.environ["RENFORGE_SAY_WHAT_SCREENSHOT"]).resolve()
                    if os.environ.get("RENFORGE_SAY_WHAT_SCREENSHOT")
                    else None
                ),
            )
